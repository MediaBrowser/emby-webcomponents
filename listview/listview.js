define(['dom', 'itemShortcuts', 'itemHelper', 'mediaInfo', 'indicators', 'connectionManager', 'layoutManager', 'globalize', 'datetime', 'apphost', 'css!./listview', 'emby-ratingbutton', 'emby-playstatebutton', 'embyProgressBarStyle'], function (dom, itemShortcuts, itemHelper, mediaInfo, indicators, connectionManager, layoutManager, globalize, datetime, appHost) {
    ...
    function getListItemHtml(item, options) {
        ...
        var clickEntireItem = options.clickEntireItem;
        var isLargeStyle = options.isLargeStyle;
        var enableContentWrapper = options.enableContentWrapper;
    ...
    function getItemParts(item, options) {
        ...
        return getListItemHtml(item, options);
    }

    function setListOptions(items, options) {

        options.enableContentWrapper = options.enableOverview && !layoutManager.tv;
        options.containerAlbumArtistIds = (options.containerAlbumArtists || []).map(getId);
        options.clickEntireItem = layoutManager.tv ? true : false;
        options.isLargeStyle = options.imageSize === 'large';
        ...
        if (options.enableContentWrapper) {
    ...
    function getListViewHtml(items, options) {
        ...
        for (var i = 0, length = items.length; i < length; i++) {
            ...
            html += getListItemHtml(item, options);
        }
